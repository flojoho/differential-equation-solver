...
const toCanvasCoordinates = (vector) => {
    const { x, y } = vector;
    const scaleX = renderWidth / (xMax - xMin);
    const scaleY = renderHeight / (yMax - yMin);
    return new Vector((x - xMin) * scaleX + padding, canvas.height - ((y - yMin) * scaleY + padding));
};
const drawLine = (vector1, vector2) => {
...
const render = () => {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        lineTo(new Vector(currentX, f(currentX)));
    }
    ctx.stroke();
};
render();
